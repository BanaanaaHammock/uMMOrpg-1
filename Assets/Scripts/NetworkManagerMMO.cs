...
public class NetworkManagerMMO : NetworkManager {
    ...
    void OnClientReceivePopup(NetworkMessage netMsg) {
        ...
        // show a popup
        GetComponent<NetworkManagerMMOUI>().ShowPopup(msg.text);
    ...
    public override void OnStartServer() {
        GetComponent<NetworkManagerMMOUI>().HideLoginMask();
    ...
    void OnClientCharactersAvailable(NetworkMessage netMsg) {
        ...
        GetComponent<NetworkManagerMMOUI>().HideLoginMask();
        GetComponent<NetworkManagerMMOUI>().HideCharacterCreation();
        GetComponent<NetworkManagerMMOUI>().characters = msg.characters;
        GetComponent<NetworkManagerMMOUI>().ShowCharacterSelection();
    }
    ...
    // player saving ///////////////////////////////////////////////////////////
    void CreateCharacter(string account, string charName, Player playerPrefab) {
        ...
        for (int i = 0; i < player.inventorySize; ++i)
            if (i < playerPrefab.defaultItems.Length)
    ...
    public override void OnServerDisconnect(NetworkConnection conn) {
        ...
        if (conn.playerControllers.Count > 0) {
            // gameobject not destroyed yet?
            if (conn.playerControllers[0].gameObject != null) {
                // get the player, save it
                var go = conn.playerControllers[0].gameObject;
                var player = go.GetComponent<Player>();
                Database.CharacterSave(player);
                print("saved:" + player.name);
            } else {
                print("couldnt save because GO already destroyed" + conn);
            }
        } else {
            print("couldnt save because conn already destroyed" + conn);
        }
    ...
    public override void OnClientDisconnect(NetworkConnection conn) {
        ...
        GetComponent<NetworkManagerMMOUI>().ShowPopup("Disconnected.");

        // show login mask again
        GetComponent<NetworkManagerMMOUI>().ShowLoginMask();
